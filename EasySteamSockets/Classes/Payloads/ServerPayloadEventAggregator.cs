...
{
    public class ServerPayloadEventAggregator
    {
        ...
        {
            ServerPayloadEventAggregator.bytePayloadTypes = bytePayloadTypes.ToArray();
            ServerPayloadEventAggregator.jsonPayloadTypes = jsonPayloadTypes.ToArray();
        }


        // Delegates

        public delegate void PayloadArrivedEventHandler<T>(T receivedPayload, NetIdentity identity) where T : AbstractPayload;
        ...
        public ServerPayloadEventAggregator()
        {
        ...
        public void Notify<T>(T payload, NetIdentity identity) where T : AbstractPayload
        {
            ...
            if (events.TryGetValue(type, out Delegate value))
            {
                ...
                dynamicHandler(dynamicPayload, identity);
            }
        }
    }
}
